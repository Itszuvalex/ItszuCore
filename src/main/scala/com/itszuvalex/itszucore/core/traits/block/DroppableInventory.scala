package com.itszuvalex.itszucore.core.traits.block
    ...
    if (shouldDrop) {
      world.getTileEntity(x, y, z) match {
        case ti: IInventory =>
          ...
          (0 until ti.getSizeInventory).map(ti.getStackInSlotOnClosing).foreach(InventoryUtils.dropItem(_, world, x, y, z, random))
