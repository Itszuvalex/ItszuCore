...
import com.itszuvalex.itszucore.api.utility.TileSave;
...
//			saveable.set(obj, info);
    ...
    static {
        ...
        registerSaveManager(UUID.class, new SaveManager() {
            ...
            public void readFromNBT(NBTTagCompound compound, Field saveable,
                ...
                try {
                    ...
                    id = PlayerUUIDTracker.getUUID(stored);
                }
                saveable.set(obj, id);
            }
        });
        ...
        registerSaveManager(TileSave.class, new SaveManager() {
            ...
            public void readFromNBT(NBTTagCompound compound, Field saveable,
                ...
                NBTTagCompound container = compound.getCompoundTag(anno.tag().isEmpty() ? saveable.getName() : anno
                        .tag());
                TileSave stack = TileSave.loadFromNBT(container);
                saveable.set(obj, stack);
            }
        });
        //
    ...
    private static void saveToNBT(NBTTagCompound compound, Field saveable,
        ...
        try {
            ...
            if (man != null) {
                ...
                ItszuCore.logger().log(Level.ERROR,
                        "Field "
                        + saveable.getName()
                        + " in Class "
