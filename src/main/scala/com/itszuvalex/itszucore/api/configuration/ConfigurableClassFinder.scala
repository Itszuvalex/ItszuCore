...
class ConfigurableClassFinder(val classPackage: String, val configKey: String = "Class Constants") {
  ...
  def registerConfigurableClasses() {
    ItszuCore.logger.log(Level.INFO, "Finding all configurable classes for registration.")
            ...
            if (clazz.getAnnotation(classOf[Configurable]) != null) {
              try {
                ...
                if (inst != null) {
                  ...
                  ItszuCore.logger.log(Level.INFO, "Registered " + clazz.getSimpleName + " as configurable singleton.")
                }
              }
              catch {
                case e: ClassNotFoundException =>
                  registerConfigurableClass(clazz)
                  ItszuCore.logger.log(Level.INFO, "Registered " + clazz.getSimpleName + " as configurable.")
                case e: Exception =>
              }
            }
          }
    ...
    ItszuCore.logger.log(Level.INFO, "Registered " + configurableClasses.length + " configurable classes.")
    ItszuCore.logger.log(Level.INFO, "Registered " + configurableSingletons.size + " configurable singletons.")
  }
